...
Although neural networks have been proposed to learn a language model by \textcite{xu_can_2000}, the main contribution of \citeauthor{bengioneural2003} is to use an \emph{embedding layer}, in order to attack \emph{curse of dimensionality}.
...
word2vec package~\cite{mikolov_efficient_2013,mikolovDistributed2013,mikolov_linguistic_2013} popularized word embeddings.
...
In \citetitle{mikolovDistributed2013}, it is defined as follows;
\begin{equation}
...
Let us turn our focus towards \citetitle{bojanowskiEnriching2016a}~\cite{bojanowskiEnriching2016a}.
Instead of using a context window to learn the representation of the target word or predicting surrounding words given the centre word like in the the skip-gram model, \citeauthor{bojanowskiEnriching2016a} learn representations for character n-grams.
...
As the number of items in the WordNet grew, only then short glosses, later followed by longer definitions got included in WordNet~\cite{fellbaumWordNet1998}.
...
\begin{table}[htbp]
    ...
    \begin{tabulary}{\textwidth}{LL}
        ...
        \bottomrule %
    \end{tabulary}
    \caption{Example synsets and their respective glosses from WordNet}%
...
Arguably, EuroWordNet set the standard for creating wordnets for languages other than English~\cite{vossenIntroduction1998, vossen_eurowordnet_2004}.
...
% \begin{enumerate}
% \item Expand model, \textcite{vossenIntroduction1998}, fixed synsets are translated from English to target language.
...
% \end{enumerate}
Two approaches for wordnet generation were defined by \citetitle{vossenIntroduction1998};
\begin{description}
...
% Just English
Further approaches using parallel corpora to align the target language with English Princeton WordNet were used by \textcite{sagotBuilding2008} in order to create a production ready French wordnet and by \textcite{fiserLeveraging2009} to create a Slovene wordnet.
...
In their paper \citetitle{khodakAutomated2017} they present 3 approaches and compare their precision, recall and coverage.
