...
The distance between two words can be represented by the Eucledian distance between them over $d$ dimensions, or;
\begin{displaymath}
    d(w_a, w_b) = || w_a - w_b ||_2
\end{displaymath}
...
This flow matrix is called T and an element $T_{i,j}$ is the distance needed to travel when starting from word $i$ to reach word $j$.
